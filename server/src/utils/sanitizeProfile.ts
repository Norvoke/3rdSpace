...
export function sanitizeCustomHTML(html: string): string {
  return sanitizeHtml(html, {
    ...
    // No `style` or `class` attribute anywhere — customCSS is the dedicated
    // styling channel; inline style= is a redundant injection surface.
    // No on* handlers — deliberately omitted, not an oversight.
    allowedAttributes: {
      a: ['href', 'target', 'rel'],
